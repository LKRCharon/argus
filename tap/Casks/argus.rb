cask "argus" do
  version "0.7.3"
  sha256 "a3d4c60c83eb6d219a9b6478a38dd30542775e5f83646930819abfedf5dd52d4"
  ...
  homepage "https://github.com/LKRCharon/argus"
  ...
  # ADR-0007 §E — symlink the same Mach-O so users can type `argus on`.
  binary "#{appdir}/Argus.app/Contents/MacOS/Argus", target: "argus"
  ...
  # a deleted Argus.app. We only touch the marker-delimited blocks our
  # installer wrote; the rest of each user config is preserved.
  uninstall_preflight do
    sed_block_args = ["-i", "", "/# >>> argus-hook/,/# <<< argus-hook/d"]
    ...
    # tagged with `"_argus": true` (HookInstaller) via python3 (proposal §6).
    ...
    claude_cleanup = <<~PYTHON
      ...
      for k in ("PreToolUse", "PostToolUse"):
          ...
          if isinstance(arr, list):
              kept = [e for e in arr if not (isinstance(e, dict) and e.get("_argus"))]
              if len(kept) != len(arr):
                  hooks[k] = kept; changed = True
      if root.pop("_argus_hook_version", None) is not None: changed = True
      if changed:
          root["hooks"] = hooks
  ...
  uninstall quit: "com.kairong.argus"
  ...
  zap trash: [
    "~/Library/Preferences/com.kairong.argus.plist",
    "~/Library/Caches/com.kairong.argus",
    "~/Library/Application Support/com.kairong.argus",
    # pre-rename leftovers (Electronic Clam / eclam) — cleanup for upgrading users
