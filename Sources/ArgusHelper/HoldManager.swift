...
/// ADR-0025 — CLI TTL hold (`argus on [--for <dur> | --forever]`)의 소유자.
///
...
final class HoldManager {
    ...
    private let log = Logger(subsystem: "com.kairong.argus", category: "hold")
    private let lock = NSLock()
    private let timerQueue = DispatchQueue(label: "com.kairong.argus.hold")
    private var timer: DispatchSourceTimer?
    private var holdUntil: Date?       // 유한 hold 의 만료 시각
    private var holdForever = false    // --forever

    private static let stateDir  = "/Library/Application Support/com.kairong.argus"
    ...
    /// (`argus off` 는 cancel 후 `setSleepDisabled(false)` 를 따로 보낸다).
    func cancel() {
        lock.lock()
        clearStateLocked()
        lock.unlock()
