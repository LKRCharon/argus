...
final class AgentsPaneViewController: NSViewController, NSTableViewDataSource, NSTableViewDelegate {
    ...
    private let hooksCaption = NSTextField(labelWithString:
        ...
        "Argus detects agents automatically by watching their "
    ...
    private let codexTrustHint = NSTextField(labelWithString:
        ...
        "Re-trust needed when Argus updates the hook code."))
    ...
    override func loadView() {
        ...
        colSource.headerToolTip = NSL("agents.tip.pattern",
            "The file path Argus watches to tell this agent is working. "
            + "~/ is your home folder.")
        colSource.width = 240
        ...
        // The pane keeps its single stack — the table's width is pinned to it —
        // but is presented on glass with a page title, matching the other panes.
        Glass.installFixedPage([Glass.wrap(outer)], in: root)
        // The table used to be pinned at exactly 160pt, which clipped the list
        // at ~5 rows while the window had hundreds of spare points. Give it a
        // floor and let it absorb the leftover height instead.
        let grow = scrollView.heightAnchor.constraint(equalToConstant: 600)
    ...
    func refresh() {
        // Recompute rows: M1 defaults + watched customize-only entries, then customs sorted by label.
        let allBuiltIn = AgentTrace.M1Defaults
            + AgentTrace.CustomizeOnly.filter { store.isAgentWatched($0.id) }
        let builtInIds = Set(allBuiltIn.map(\.id))
        let customs = store.customTraces.sorted { $0.label < $1.label }
        rows = allBuiltIn + customs.filter { !builtInIds.contains($0.id) }
    ...
    func tableView(_ tableView: NSTableView,
        ...
        case "watched":
            ...
            let container = tableView.makeView(withIdentifier: id, owner: nil) as? NSView
                ?? makeCenteredCell(makeCheckbox(identifier: id), identifier: id, centerX: true)
            guard let check = container.subviews.compactMap({ $0 as? NSButton }).first else { return container }
        ...
        case "name":
            ...
            let container = tableView.makeView(withIdentifier: id, owner: nil) as? NSView
                ?? makeCenteredCell(makeLabel(identifier: id), identifier: id)
            guard let label = container.subviews.compactMap({ $0 as? NSTextField }).first else { return container }
    ...
    /// Wraps a cell control in a container that vertically centers it via Auto
    /// Layout. Bare controls returned to a view-based NSTableView get stretched
    /// to the cell frame, and the checkbox's image center drifts below the text
    /// baseline (user feedback: 文字比多选框高). Centering both on the same axis
    /// fixes the row alignment. `centerX` horizontally centers the control
    /// (checkbox column); labels stay leading-aligned so truncation still works.
    private func makeCenteredCell(_ control: NSView,
                                  identifier: NSUserInterfaceItemIdentifier,
                                  centerX: Bool = false) -> NSView {
        ...
        if centerX {
            ...
            constraints.append(control.leadingAnchor.constraint(equalTo: container.leadingAnchor, constant: 2))
            constraints.append(control.trailingAnchor.constraint(lessThanOrEqualTo: container.trailingAnchor, constant: -2))
        }
    ...
    @objc private func modeRadioChanged(_ sender: NSButton) {
        ...
        // InfoButton.wrap puts each radio in its own container, breaking the
        // auto mutual-exclusion that macOS normally provides for sibling radio
        // buttons. Manually sync both states so they don't appear simultaneously on.
        strictRadio.state = (next == .strict) ? .on : .off
        laxRadio.state = (next == .lax) ? .on : .off
    ...
    @objc private func uninstallAllTapped() {
        let confirm = NSAlert()
        confirm.messageText = NSL("agents.uninstallAll.title", "Uninstall all Argus hooks?")
        confirm.informativeText = NSL("agents.uninstallAll.body",
            "Removes Argus hook entries from "
