...
///
/// Master toggle + battery slider + thermal cutoff slider + max-duration
...
final class SafetyPaneViewController: TimedRefreshPaneViewController {
    ...
    /// slider's tick positions. Logic maps by index — never localize.
    ...
    /// Discrete 4-stage slider — same ordered scale as the battery slider above
    /// it, with the selected stage named in the label on its right.
    private let thermalSlider = NSSlider(value: 1, minValue: 0, maxValue: 3, target: nil, action: nil)
    private let thermalValue = NSTextField(labelWithString: "")
    /// One-line caption under the thermal slider explaining the scale direction
    ...
    override func loadView() {
        ...
        // Cutoff slider — stage names shared with the live "Current state"
        // readout so the two scales line up; the label carries a direction hint
        // on the extremes. Separate keys from `thermal.*` so the live labels
        // stay plain.
        thermalSlider.target = self
        thermalSlider.action = #selector(thermalChanged)
        thermalSlider.allowsTickMarkValuesOnly = true
        thermalSlider.numberOfTickMarks = Self.thermalValues.count
        thermalSlider.translatesAutoresizingMaskIntoConstraints = false
        thermalSlider.widthAnchor.constraint(equalToConstant: 220).isActive = true
        thermalValue.font = NSFont.systemFont(ofSize: 11)
        let thermalRow = NSStackView(views: [thermalSlider, thermalValue])
        thermalRow.orientation = .horizontal
        thermalRow.spacing = 12
        ...
        // Floor only — the enclosing section stretches the chart to the card
        // width, and 460 as a hard minimum overflowed the card at the window's
        // minimum size (560pt wide leaves ~496pt inside the padding).
        thermalChart.widthAnchor.constraint(greaterThanOrEqualToConstant: 280).isActive = true

        // One glass card per guard, so each limit reads as its own unit instead of
        // relying on whitespace to imply grouping.
        Glass.installPage([
            Glass.section("", [
                InfoButton.wrap(masterCheckbox, tips.master),
                notifyCheckbox,
            ]),
            Glass.section(NSL("safety.card.battery", "Battery"), [
                InfoButton.wrap(batteryHeader, tips.battery),
                batteryRow,
            ]),
            Glass.section(NSL("safety.card.thermal", "Temperature"), [
                InfoButton.wrap(thermalHeader, tips.thermal),
                thermalRow,
                thermalCaption,
            ]),
            Glass.section(NSL("safety.card.duration", "Awake time"), [
                InfoButton.wrap(durationHeader, tips.duration),
                durationPopup,
            ]),
            Glass.section(NSL("safety.card.state", "Current state"), [
                InfoButton.wrap(stateHeader, tips.state),
                stateBody,
                thermalChart,
            ]),
            Glass.section(NSL("safety.card.release", "Last release"), [
                InfoButton.wrap(releaseHeader, tips.release),
                releaseBody,
            ]),
        ], in: root)
        self.view = root
    ...
    private func installTooltips()
        ...
        let masterTip = NSL("safety.tip.master",
            "The master switch for every guard below. When on, Argus "
        ...
        thermalSlider.toolTip = thermalTip
    ...
    func refresh() {
        ...
        thermalSlider.intValue = Int32(thermalIndex)
        thermalValue.stringValue = Self.thermalCutoffTitle(at: thermalIndex)
    ...
    @objc private func thermalChanged() {
        let idx = Int(thermalSlider.intValue)
        guard idx >= 0, idx < Self.thermalValues.count else { return }
        thermalValue.stringValue = Self.thermalCutoffTitle(at: idx)
        var s = store.safetySettings
        // Live-drag emits repeated identical values once the knob snaps — only
        // persist on an actual change.
        guard s.thermalCutoff != Self.thermalValues[idx] else { return }
        s.thermalCutoff = Self.thermalValues[idx]
        store.updateSafetySettings(s)
    }

    /// Localized stage title for a slider position, with direction hints on
    /// the extremes (same keys the old dropdown used).
    private static func thermalCutoffTitle(at index: Int) -> String {
        switch index {
        case 0: return NSL("thermal.cutoff.nominal", "Nominal — soonest, safest")
        case 1: return NSL("thermal.cutoff.fair", "Fair")
        case 2: return NSL("thermal.cutoff.serious", "Serious")
        default: return NSL("thermal.cutoff.critical", "Critical — latest")
        }
    }
