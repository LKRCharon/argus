...
final class MenuBarController: NSObject, NSMenuDelegate {
    ...
    private func renderStatusButton() {
        ...
        if !on {
            ...
            state = .bolt
            fallbackSymbol = "link.circle.fill"
        ...
        -> (text: String, symbol: String)
    {
        switch status {
        case .enabled:
            return (enabledHeader(), "link")
        case .requiresApproval:
            return (NSL("header.requiresApproval", "Approve in System Settings…"), "lightbulb")
        case .notFound:
            return (NSL("header.notFound", "Helper missing — reinstall"), "link")
    ...
    private func enabledHeader() -> String {
        ...
        // 5¾. Manual pin — explicit user intent, same rule as the CLI hold
        // above: it outranks the ambient remote/agent signals. This also keeps
        // the header consistent with the menu-bar glyph, where the bolt is
        // reserved for manualToggle (the two used to disagree: bolt icon +
        // "Awake — remote session" header for the same state).
        if store.shouldKeepAwake && store.manualToggle {
        ...
        // 8. Catch-all awake (safety hold, stale hold, …).
        if store.shouldKeepAwake {
            return NSL("header.awakeManual", "Awake — until I quit")
        }
    ...
    private func buildAgentsSubmenu() -> NSMenu {
        ...
        // Stable display order: M1 defaults + watched customize-only, then customs sorted by label.
        ...
        let ordered: [AgentTrace] = allBuiltIn + customs.filter { !builtInIds.contains($0.id) }
    ...
    @objc private func toggleTapped() {
        ...
        case .notRegistered, .registerThrew:
            ...
            refresh()
        case .notFound:
            if let url = URL(string: "https://github.com/kairong/argus#readme") {
