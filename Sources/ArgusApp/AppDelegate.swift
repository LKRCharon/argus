...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private let log = Logger(subsystem: "com.kairong.argus", category: "app")
    ...
    private var agentlinkBridge: AgentlinkBridge?
    /// User-level sleep prevention (ProcessInfo.beginActivity) — the primary
    /// keep-awake mechanism. The root helper is now an optional enhancement.
    private let activityAssertion = ActivityAssertion()
    ...
    private let heartbeatQueue = DispatchQueue(label: "com.kairong.argus.heartbeat")
    ...
    /// Walks the view tree reporting anything suspicious: zero width/height, or
    /// a frame that sticks out past its window. Debug aid for `--dump-layout`.
    private static func dumpLayout(_ view: NSView, bounds: NSRect, depth: Int) {
        for sub in view.subviews {
            var flags: [String] = []
            if sub.frame.width < 1 { flags.append("ZERO-W") }
            if sub.frame.height < 1 { flags.append("ZERO-H") }
            if !sub.isHidden, let win = sub.window {
                let wb = win.contentView?.bounds ?? .zero
                let local = sub.convert(sub.bounds, to: win.contentView)
                if local.maxX > wb.maxX + 1 { flags.append("OVERFLOW-X(\(Int(local.maxX - wb.maxX)))") }
                if local.minX < wb.minX - 1 { flags.append("OFFSCREEN-X") }
                if local.maxY > wb.maxY + 1 || local.minY < wb.minY - 1 { flags.append("CLIP-Y") }
            }
            if !flags.isEmpty && !sub.isHidden {
                let name = String(describing: type(of: sub))
                print("  \(String(repeating: "  ", count: depth))\(name) "
                      + "\(Int(sub.frame.width))x\(Int(sub.frame.height)) \(flags.joined(separator: ","))")
            }
            dumpLayout(sub, bounds: bounds, depth: depth + 1)
        }
    }
    ...
    private func presentApprovalOnboardingIfNeeded() {
        ...
        alert.informativeText = NSL("onboarding.message",
            "Argus needs you to allow its background helper in System Settings → General → Login Items. Open it now and switch Argus on.")
    ...
    private func presentInstallLocationAlertIfNeeded() -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = NSL("installgate.title", "Move Argus to your Applications folder")
        alert.informativeText = (block.kind == .quarantined)
            ? NSL("installgate.message.quarantined", "Argus is running from a download location, so macOS won’t let its background helper start. Move it to the Applications folder and open it from there.")
            : NSL("installgate.message.translocated", "macOS is running Argus from a temporary read-only location. Move it to the Applications folder and reopen it so its helper can start.")
    ...
    func applicationDidFinishLaunching(_ note: Notification) {
        ...
        // CLI↔GUI reconciliation — `argus on/off` writes the helper directly,
        ...
        // Detection and the safety guards are pure local observation (session
        // logs, battery, thermal state) and keep-awake now runs on a user-level
        // ProcessInfo activity. None of it needs the privileged helper, so none
        // of it is gated on registration any more — gating it meant an
        // unregistered helper silently disabled agent detection entirely, and
        // with it every keep-awake decision.
        detector.setTraces(store.tracesToWatch())
        detector.start()
        startRemoteAndSafety()

        if case .enabled = store.registration {
            // Helper-only concerns: the watchdog handshake and the trip reason
            // it recorded while we were disconnected.
            startHeartbeat()
            bridge.fetchLastTripReason { [weak self] reason in
                guard let self = self, let reason = reason else { return }
                // Log only. The watchdog means "the helper stopped forcing sleep
                // off because the GUI went quiet" — it says nothing about the
                // current session, and the reason it reports can be hours stale
                // from a leftover helper process. Writing it into `safetyRelease`
                // used to kill keep-awake outright (plus a 5-minute cooldown) on
                // every launch that found such a helper, even though keep-awake
                // no longer runs through the helper at all.
                self.log.notice("helper last trip reason: \(reason, privacy: .public)")
            }
        }
        menuBar.refresh()
        ...
        if let i = launchArgs.firstIndex(of: "--open-settings") {
            let index = launchArgs.count > i + 1 ? Int(launchArgs[i + 1]) ?? 0 : 0
            let pane = SettingsWindowController.Pane(rawValue: index) ?? .general
            DispatchQueue.main.async { [weak self] in self?.openSettings(pane: pane) }
        }
    }
    ...
    private func startHeartbeat() {
        ...
        t.setEventHandler { [weak self] in
            ...
            // believe we're holding sleep off, so an out-of-band `argus off` is
    ...
    /// `pane: nil` → restore the most recently viewed pane (HIG).
    private func openSettings(pane: SettingsWindowController.Pane? = nil) {
        if settingsWindow == nil {
            settingsWindow = SettingsWindowController(store: store, history: history, agentlinkBridge: agentlinkBridge!,
    ...
    private func startSubsystemsIfNewlyEnabled() {
        ...
        if sawUnapprovedThisRun,
           !UserDefaults.standard.bool(forKey: Self.onboardingCelebratedKey) {
            ...
            let title = NSL("onboarding.enabled.title", "Argus is ready")
    ...
    private func convergeNow() {
        // Keep the watched-traces set in sync — cheap. Runs regardless of helper
        ...
        // SleepDisabled setting further below.
        let keepAwakeTarget = store.shouldKeepAwake
        if keepAwakeTarget {
            activityAssertion.begin(reason: "Argus: agent activity keeping the Mac awake")
        } else {
