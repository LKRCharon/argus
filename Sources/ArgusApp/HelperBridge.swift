...
final class HelperBridge {
    private let log = Logger(subsystem: "com.kairong.argus", category: "xpc")
    ...
    // Circuit breaker. With keep-awake no longer running through the helper, the
    // helper is frequently absent (unregistered, or deliberately uninstalled) —
    // and every call used to build a fresh connection that invalidated
    // immediately, spinning through connect/invalidate several times a second.
    // After `breakerThreshold` consecutive invalidations we stop dialing for
    // `breakerCooldown`, which keeps the log and the CPU quiet while leaving a
    // path back once a helper does appear.
    private var consecutiveInvalidations = 0
    private var breakerOpenUntil: Date?
    private static let breakerThreshold = 3
    private static let breakerCooldown: TimeInterval = 60

    init(store: StateStore) {
        self.store = store
    }

    /// Returns a live connection, or `nil` while the breaker is open (no helper
    /// to talk to). Callers already treat a missing proxy as "helper
    /// unavailable", so opening the breaker degrades gracefully.
    private func ensureConnection() -> NSXPCConnection? {
        lock.lock(); defer { lock.unlock() }
        if let c = connection { return c }
        if let until = breakerOpenUntil {
        ...
        c.invalidationHandler = { [weak self] in
            ...
            self.lock.lock()
            self.connection = nil
            self.consecutiveInvalidations += 1
            let tripped = self.consecutiveInvalidations >= Self.breakerThreshold
            if tripped { self.breakerOpenUntil = Date().addingTimeInterval(Self.breakerCooldown) }
            self.lock.unlock()
            if tripped {
                self.log.notice("helper unreachable; pausing XPC for \(Int(Self.breakerCooldown))s")
            } else {
    ...
    /// Called when the helper actually answers. Only a real reply clears the
    /// breaker — merely creating a connection proves nothing, since invalidation
    ...
    /// (`argus on/off` talks to the helper directly) and drop its
