...
final class GeneralPaneViewController: NSViewController {
    ...
    /// re-rendered by `renderHelperUnreachableNote()` from `store.registration` on show
    ...
    /// (CLI `argus status` 신호를 터미널 안 쓰는 유저에게도). mdfind/launchctl 을
    ...
    /// 레코드가 제거된/중복 복사본에 묶인 2026-07-01 사건), CLI `argus repair` 가 주는
    ...
    /// Status card backdrop. Draws the Tahoe glass material where available and
    /// falls back to the flat filled+stroked rectangle on macOS 13-15, so the
    /// card looks native on every supported system.
    private final class CardView: NSView {
        private var backdrop: NSView?

        override func viewDidMoveToWindow() {
            super.viewDidMoveToWindow()
            guard backdrop == nil, #available(macOS 26.0, *) else { return }
            let v = Glass.card(cornerRadius: 10)
        ...
        override func draw(_ dirtyRect: NSRect) {
            // Glass backdrop installed — nothing to paint ourselves.
    ...
    override func loadView() {
        ...
        let loginItemTip = NSL("general.tip.openAtLogin",
            "Launch Argus automatically when you log in, so it's always watching in the menu bar. You can also manage this in System Settings → General → Login Items.")
        ...
        clamshellGuardCheckbox.toolTip = NSL("general.tip.clamshellGuard",
            ...
            + "Argus keeps an invisible virtual display alive so macOS "
        ...
        vpnNotifyCheckbox.toolTip = NSL("general.tip.vpnNotify",
            "While the Mac is kept awake, Argus watches your VPN with scutil "
        ...
        let vpnServiceTip = NSL("general.tip.vpnService",
            ...
            + "is usually \"VPN\". Argus watches it with scutil and, if it drops, "
        ...
        helperUnreachableNote.isHidden = true   // renderHelperUnreachableNote() 가 상태에 맞게 갱신
        ...
        // ADR — keep-awake runs on a user-level ProcessInfo activity, so the
        // privileged helper is no longer required for the app to work. Its
        // registration UI (status line, reinstall, diagnostics) was pure noise
        // for that reason and is gone; the helper itself stays for the CLI's TTL
        // holds and the hook's instant-detection path, and reports problems
        // through the notes below only when something actually breaks.
        ...
        let settingsStack = NSStackView(views: [
            ...
            diagnosticsRow,
            helperUnreachableNote, resetBtmNote,
        ])
        ...
        } else if let symbol = NSImage(systemSymbolName: "link.circle.fill", accessibilityDescription: "Argus") {
        ...
        let nameLabel = NSTextField(labelWithString: "Argus")
        ...
        let linkButton = NSButton(title: "github.com/kairong/argus", target: self, action: #selector(openRepo))
        ...
        let linksRow = NSStackView(views: [linkButton])
        ...
        NSLayoutConstraint.activate([
            ...
            // Warning notes wrap within the form width.
    ...
    private func renderStatusCard() {
        ...
        statusTitle.stringValue = on
            ? NSL("general.statusCard.on", "Argus — On · staying awake")
            : NSL("general.statusCard.off", "Argus — Off · sleeps normally")
        // Helper registration is no longer the thing that makes keep-awake work,
        // so the card's subtitle reports what is actually holding the Mac awake.
        statusHelper.stringValue = store.shouldKeepAwake
            ? NSL("general.statusCard.reason", "An agent is working — sleep is held off")
            : NSL("general.statusCard.idle", "No agent activity detected")
    }

    /// The one helper condition still worth surfacing: registered but not
    /// answering XPC. Keep-awake itself is unaffected (it runs on a ProcessInfo
    /// activity now), so this only warns that the CLI's timed holds won't work.
    private func renderHelperUnreachableNote() {
    ...
    @objc private func openRepo() {
        if let url = URL(string: "https://github.com/kairong/argus") {
