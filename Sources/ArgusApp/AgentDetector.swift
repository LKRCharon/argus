...
enum LaxProcessAlive {
    ...
    private static let scanQueue = DispatchQueue(label: "com.kairong.argus.lax.ps")
    ...
    private static func scan() -> Set<String> {
        ...
        for raw in s.split(separator: "\n") {
            ...
            // Qoder is an Electron app; its main process comm is the generic
            // "Electron". Detect it by app-bundle path so Lax mode can keep
            // awake while Qoder is running without matching every Electron app.
            if trimmed.contains("/Qoder.app/") { out.insert("Qoder") }
        }
        return out
    }
}
...
final class AgentDetector {
    private let log = Logger(subsystem: "com.kairong.argus", category: "agent")
    ...
    /// Guards against overlapping async ticks. When the background glob+stat
    /// is still in flight, the next timer fire is skipped rather than queued.
    ...
    private let procQueue = DispatchQueue(label: "com.kairong.argus.agent.proc")
    ...
    func setTraces(_ next: [AgentTrace]) {
        ...
        // Skip re-evaluation if the trace set hasn't changed. convergeNow()
        // calls this on every store.onChange (e.g. mode switches, agent
        // toggles), and the full tick() below does synchronous glob(3)+stat()
        // for every trace on the main thread — that I/O causes UI stutter
        // when traces didn't actually change. The 5s polling timer already
        // keeps activity detection fresh.
        if collapsed.sorted(by: { $0.id < $1.id }) == self.traces.sorted(by: { $0.id < $1.id }) {
            return
        }
    ...
    private func tick() {
        ...
        let now = Date()
        let tracesSnapshot = traces
        let hasHooks = tracesSnapshot.contains { $0.hookKey != nil }

        // Offload I/O-heavy work (glob+stat, PID scan, session scan) to a
        // background queue. The main-thread-only evaluation (hook state,
        // Claude pairing, watcher reconciliation) runs after on main.
        DispatchQueue.global(qos: .utility).async { [weak self] in
            guard let self = self else { return }

            // Pre-compute glob matches — the I/O-heavy part.
            var precomputed: [(String, (String, TimeInterval)?)] = []
            for trace in tracesSnapshot {
                let pair = self.freshestMatch(pattern: trace.globPattern, now: now)
                precomputed.append((trace.id, pair))
            }
            let matchMap = Dictionary(precomputed, uniquingKeysWith: { _, b in b })

            // PID file scan — skip when no hooks are configured (no PID
            // files would exist). Saves a directory listing + stat per tick.
            let pidFileSources = hasHooks ? self.scanPIDFiles(now: now) : []

            // Session watcher scan (cheap directory listing).
            let sessionIDs = self.sessionWatcher.aliveSessionIDs(now: now)

            DispatchQueue.main.async { [weak self] in
                guard let self = self else { return }
                self.tickInFlight = false

                let liveClaude = self.liveClaudeWorkspaces(now: now)
                var nextActive: Set<String> = []

                for trace in self.traces {
                    let pair = matchMap[trace.id] ?? nil
                    let row = self.evaluateTraceWithMatch(trace, now: now,
                                                           match: pair,
                                                           liveClaude: liveClaude,
                                                           pidFileSources: pidFileSources)
                    if row.active { nextActive.insert(trace.id) }
                }
                nextActive.formUnion(sessionIDs)

                self.reconcileWatchers()

                if nextActive != self.active {
                    self.active = nextActive
                    self.onChange?(nextActive)
                }
            }
        }
    }
    ...
    private func evaluateTrace(_ trace: AgentTrace,
        ...
        // Compute glob match synchronously (used by revalidateMtime and debugSnapshot).
        let match = freshestMatch(pattern: trace.globPattern, now: now)
        return evaluateTraceWithMatch(trace, now: now, match: match,
                                      liveClaude: liveClaude, pidFileSources: pidFileSources)
    }

    /// Evaluate with a pre-computed glob match (used by the async tick path).
    /// The match is computed off-main; this method runs on main to access
    /// `lastHookPing` and `liveClaudeWorkspaces` safely.
    private func evaluateTraceWithMatch(_ trace: AgentTrace,
                                         now: Date,
                                         match: (String, TimeInterval)?,
                                         liveClaude: Set<String>,
                                         pidFileSources: Set<String>) -> AgentDetectorDebugSnapshot.TraceRow {
        ...
        // File-mtime channel — use the pre-computed match.
        let pair = match
...
final class SessionWatcher {
    ...
    private let log = Logger(subsystem: "com.kairong.argus", category: "session")
