...
final class StateStore {
    ...
    struct SafetySettings: Codable, Equatable {
        ...
        // `serious`, not `fair`: an agent working the CPU puts a laptop at
        // `.fair` almost immediately, so a fair cutoff released keep-awake on
        // essentially every real session (observed 2026-07-27). `serious` is the
        // first level that means the machine is actually in trouble.
    ...
    /// One-shot marker for the qoder promotion (v0.6.x — CustomizeOnly → default).
    /// Stored watched-sets predate the promotion, and Qoder was never listed in
    /// the default table, so its absence can't be a deliberate opt-out: insert
    /// once; the marker makes a later deliberate uncheck stick.
    private static let qoderPromotedKey = "WatchedAgentsQoderPromoted"
    ...
    init() {
        ...
        if !UserDefaults.standard.bool(forKey: Self.qoderPromotedKey) {
            if !watchedAgents.contains("qoder") {
                watchedAgents.insert("qoder")
                UserDefaults.standard.set(Array(watchedAgents).sorted(), forKey: Self.watchedAgentsKey)
            }
            UserDefaults.standard.set(true, forKey: Self.qoderPromotedKey)
        }
    ...
    func allKnownTraces() -> [AgentTrace] {
        ...
        for t in AgentTrace.CustomizeOnly { byId[t.id] = t }
