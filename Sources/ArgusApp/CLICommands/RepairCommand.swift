...
/// `argus repair` — recover a wedged/unreachable helper by relaunching the app,
...
enum RepairCommand: CLISubcommand {
    static func run(args: [String]) -> Int32 {
        ...
        if let extra = args.first {
            CLIStderr.print("argus repair: unexpected argument '\(extra)' (repair takes no arguments).")
            return 1
        }
        ...
        if copies.count > 1 {
            print("argus repair: multiple installs detected (split-brain risk). Keep only the one in /Applications and remove the rest:")
        ...
        if reg == .enabled, HelperLiveness.isReachable(timeout: 3.0) {
            print("argus repair: helper already registered and reachable — nothing to do.")
            return 0
        }
        ...
        if reachable {
            print("argus repair: helper is reachable again.")
            return 0
        }
        ...
        if post == .requiresApproval {
            CLIStderr.print("argus repair: the helper needs approval. Open System Settings > General > "
                + "Login Items & Extensions, enable Argus, then re-run `argus repair`.")
            return 3
        }
        CLIStderr.print("argus repair: still unreachable after relaunch. Open Argus and use "
        ...
        CLIStderr.print("If it persists, a stale background record may be bound to a removed copy. "
            + "Last resort: 'sudo sfltool resetbtm', then reboot and reopen Argus "
    ...
    Recover a registered-but-unreachable helper by relaunching Argus,
