...
/// `argus on [--for <dur>] [--forever]` — ADR-0025 CLI TTL hold.
///
        ...
        while i < args.count {
            ...
            case "--for":
                guard i + 1 < args.count,
                    ...
                    CLIStderr.print("argus on: --for needs a duration like 30m, 2h, 1h30m")
                    return 1
                }
                seconds = s
                i += 1
            default:
                CLIStderr.print("argus on: unknown argument '\(args[i])' (try --for <dur> or --forever)")
                return 1
            }
            i += 1
        }
...
enum SleepDisabledRPC {
    ...
    /// ADR-0025 — `argus off`: hold 활성 중의 off 쓰기는 helper 가 무시하므로
